...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Main Lambda handler for Supply Off complaint flow
    Handles flow from Step 6 onwards as per diagram
    """
    try:
...
def handle_business_rule_failure(dkt_status: str, reason: str, docket_no: Optional[str]) -> Dict[str, Any]:
    ...
    # Map status codes to specific cases (based on diagram)
...
def handle_existing_docket(docket_no: str, params: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Return existing docket info without status
        return bedrock_wrap("SupplyOff", "SupplyOffComplaint", {
    ...
    # Determine if docket is complete or incomplete
    ...
    # Add vehicle location if available (for incomplete dockets)
...
def validate_complaint_type(comp_type: str) -> bool:
    """Validates complaint type"""
    # "1": No Power, "14": Voltage Complaint
    valid_types = ["1", "14"]
